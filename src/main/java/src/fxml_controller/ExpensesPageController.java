...
public class ExpensesPageController extends FXMLControllerTemplate {
    ...
    /**
     * Adds all Expenses to ALL the expense tabs.
     */
    ...
    /**
     * Adds all Expenses to the TableViews in each Pay Period tab for the given
     * expense year.
     */
    private void populatePayPeriodTabs() {

        // Adds every Expense of the given ExpenseType to the expense tab
        for (Expense expense : expenseController.getExpensesByExpenseType(ExpenseType.EXPENSE)) {
            addExpenseToTable(expense);
        }
